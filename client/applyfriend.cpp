    ...
    ui(new Ui::ApplyFriend),_label_point(2,6)
{
    ...
    InitTipLbs();
    //链接输入标签回车事件
    connect(ui->lb_ed, &CustonizeEdit::returnPressed, this, &ApplyFriend::SlotLabelEnter);
    connect(ui->lb_ed, &CustonizeEdit::textChanged, this, &ApplyFriend::SlotLabelTextChange);
    connect(ui->lb_ed, &CustonizeEdit::editingFinished, this, &ApplyFriend::SlotLabelEditFinished);
...
void ApplyFriend::SlotApplySure()
{
    ...
    // //发送tcp请求给chat server
    // emit TcpMgr::get_instance()->sig_send_data(ReqId::ID_ADD_FRIEND_REQ, jsonString);
    this->hide();
    deleteLater();
}
